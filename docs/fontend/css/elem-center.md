...
* [元素垂直居中](#元素垂直居中)
  - [方法-1 设置`line-height`等于父元素的高度](#方法-1-设置`line-height`等于父元素的高度)
  - [方法-2 `vertical-align:middle`结合`display:inline-block`](#方法-2-`vertical-align:middle`结合`display:inline-block`)
  - [方法-3 `vertical-align:middle`结合`display:table`](#方法-3-`vertical-align:middle`结合`display:table`)
  * [方法-4 使用-Flex-布局(旧版)](#方法-4-使用-Flex-布局旧版)
  * [方法-5 使用-Flex-布局(新版)](#方法-5-使用-Flex-布局新版)
  * [方法-6 margin-负半值](#方法-6-margin-负半值)
  * [方法-7 设置父元素相对定位(position:relative)](<#方法-7-设置父元素相对定位(position:relative)>)
  * [方法-8 给居中元素前面放一个空块级元素](#方法-8-给居中元素前面放一个空块级元素)
  * [方法-9 使用内边距的方式使元素垂直居中](#方法-9-使用内边距的方式使元素垂直居中)
* [水平+垂直居中](#水平+垂直居中)

  - [方式-1 使用`line-height:高度`-`text-align:center`](#方式-1-使用`line-height:高度`-`text-align:center`)
  - [方式-2 margin-负半值进行元素的水平垂直居中显示](#方式-2-margin-负半值进行元素的水平垂直居中显示)
  - [方式-3 绝对定位结合-top-left-right-bottom](#方式-3-绝对定位结合-top-left-right-bottom)
  - [方式-4 使用-js-动态计算使其元素水平垂直居中](#方式-4-使用-js-动态计算使其元素水平垂直居中)
  - [方法-5 使用-jQuery-实现元素的水平垂直居中](#方法-5-使用-jQuery-实现元素的水平垂直居中)

* [两种常见布局圣杯布局与双飞翼布局](#两种常见布局圣杯布局与双飞翼布局)
  - [圣杯布局](#圣杯布局)
    - [方式-1 使用绝对定位实现圣杯布局](#方式-1-使用绝对定位实现圣杯布局)
    - [方式-2 使用浮动实现圣杯布局显示效果](#方式-2-使用浮动实现圣杯布局显示效果)
    - [方式-3 利用弹性盒模型(旧版本)](#方式-3-利用弹性盒模旧版本)
    - [方式-4 利用弹性盒模型(新版本)](#方式-4-利用弹性盒模型新版本)
  * [双飞翼布局(等高布局)](#双飞翼布局等高布局)
* [总结](#总结)
